...
print("<form action='./demo_hspf.php' method=post>");
print("<table>");
print("<tr>");
print("<td valign=top>");
...
if (isset($_POST['submit'])) {
   ...
   $hspf2channel = new timeSeriesInput;
   $hspf2channel->init();
   $hspf2channel->timer = $timer;
   $hspf2channel->name = "Coote's Store";
   $hspf2channel->debug = 0;
   $hspf2channel->intmethod = 3;
   $paramcolumns = array('Qout'=>1);
   # Coote's store
   #$plotgenfile = $compdir . '/comp1/RCHRES.cbp.1.out';
   # Mt. Jackson
   $plotgenfile = $compdir . '/comp1/RCHRES.cbp.14.out';
   createPlotgenTimeSeries($hspf2channel, $plotgenfile, $paramcolumns, 1, $startdate, $enddate, $debug);
   
   $hspfflow = new timeSeriesInput;
   $hspfflow->init();
   $hspfflow->timer = $timer;
   $hspfflow->name = "Coote's Store";
   $hspfflow->debug = 0;
   $hspfflow->intmethod = 3;
   $paramcolumns = array('Qout'=>1);
   # Coote's store
   #$plotgenfile = $compdir . '/comp2/RCHRES.localflow.1.out';
   # Mt. Jackson
   $plotgenfile = $compdir . '/comp2/RCHRES.localflow.14.out';
   createPlotgenTimeSeries($hspfflow, $plotgenfile, $paramcolumns, 1, $startdate, $enddate, $debug);
   #print_r($hspfflow->tsvalues);
   
   # gets daily flow values for indicated period
   print("Obtaining Flow Data for station: $staid <br>");
   createUSGSTimeSeries($flow2, '', $staid, $startdate, $enddate, '', 1, $debug);
   /*
   $site_result = retrieveUSGSData($staid, '', 1, $startdate, $enddate, 1, '', 'rdb', $dataitem);
   ...
   */
   ...
   $r1->state['substrateclass'] = 'C';
   $r1->state['sp_sub_size'] = 4;
   $r1->state['pct_fines'] = 0.12;
   ...
   #$r1->addInput('Qin', 'Qout', $hspf2channel);
   ...
   */
   ...
   # add these withdrawals to the river object
   ...
   # water temperature habitat index
   $smb->addInput('Temp', 'Temp', $flow2);
   $thisop = new lookupObject;
   $thisop->setUp('Temp', 1, array('0.0'=>0.0,'2.0'=>0.0,'24.0'=>1.0,'29.5'=>1.0, '32.0'=>0.0), 0.0);
   $smb->addOperator('V10', $thisop, 0.0);
   # pH habitat index
   $smb->addInput('pH', 'pH', $flow2);
   $thisop = new lookupObject;
   $thisop->setUp('pH', 1, array('0.0'=>0.0,'3.0'=>0.0,'5.0'=>0.2,'6.0'=>0.4,'7.0'=>0.8,'8.0'=>1.0,'8.5'=>0.8, '9.0'=>0.4, '10.0'=>0.1), 0.0);
   $smb->addOperator('V6', $thisop, 0.0);
   # water quality HSI
   $thisop = new Equation();
   $thisop->equation = ' 10000 * (V10 * V6)^(1/2) )';
   $thisop->init();
   $smb->addOperator('Vwq',  $thisop, 0);
   ...
   $thisop->init();
   $smb->addOperator('HSI20k',  $thisop, 0);
   ...
   */
   ...
   for ($i = 1; $i <= $numsteps; $i++) {
      ...
      $hsi[$smb->state['flow']]['canoe_HSI'] = $canoe->state['HSI'];
      $hsi[$smb->state['flow']]['fg_WUA'] = $ghsi->state['fg'];
      $hsi[$smb->state['flow']]['pr_WUA'] = $ghsi->state['pr'];
      $V5 = $rbt->state['V5'];
      $V7 = $rbt->state['V7'];
      $V16 = $rbt->state['V16'];
      if ($rbt->state['HSI'] < 0.1) {
         $flow = number_format($rbt->state['flow'],2);
         $si = $rbt->state['HSI'];
      ...
      $gageduration[$i]['Qout'] = $flow2->state['Qout'];
      $cbpduration[$i]['Qout'] = $hspf2channel->state['Qout'];
      $vtduration[$i]['Qout'] = $hspfflow->state['Qout'];
      ...
      # get fish kill data
      $listobject->querystring = "  select count(*) as killreport ";
      $listobject->querystring .= " from fish_kills ";
      $listobject->querystring .= " where thisdate = '$stepdate' ";
      $listobject->querystring .= " and dead_fish = 1 ";
      #print("$listobject->querystring ; <br>");
      $listobject->performQuery();
      #$listobject->showList();
      $killreport = $listobject->getRecordValue(1,'killreport');
      if ($killreport > 0) $killreport = 2000;
      array_push($fishkills, array('thisdate'=>$timer->thistime->format('r'), 'killreport'=>$killreport));
   ...
   $storage = number_format($r1->state['Storage'], 1, '.', ',');
   ...
   $thisgraph['bargraphs'][0]['graphrecs'] = $flow2->logtable;
   ...
   $thisgraph['bargraphs'][1]['graphrecs'] = $hspfflow->logtable;
   $thisgraph['bargraphs'][1]['xcol'] = 'thisdate';
   $thisgraph['bargraphs'][1]['ycol'] = 'Qout';
   $thisgraph['bargraphs'][1]['color'] = 'red';
   $thisgraph['bargraphs'][1]['ylegend'] = 'HSPF/VT Model Flow';
   
   $thisgraph['bargraphs'][2]['graphrecs'] = $hspf2channel->logtable;
   $thisgraph['bargraphs'][2]['xcol'] = 'thisdate';
   $thisgraph['bargraphs'][2]['ycol'] = 'Qout';
   $thisgraph['bargraphs'][2]['color'] = 'orange';
   $thisgraph['bargraphs'][2]['ylegend'] = 'HSPF/CBP Model Flow';
 
/*   
   $thisgraph['bargraphs'][2]['graphrecs'] = $ghsi->logtable;
   $thisgraph['bargraphs'][2]['xcol'] = 'thisdate';
...
*/   
   $thisgraph['bargraphs'][3]['graphrecs'] = $smb->logtable;
   $thisgraph['bargraphs'][3]['xcol'] = 'thisdate';
   $thisgraph['bargraphs'][3]['ycol'] = 'HSI20k';
   $thisgraph['bargraphs'][3]['color'] = 'black';
   $thisgraph['bargraphs'][3]['yaxis'] = 2;
   $thisgraph['bargraphs'][3]['ylegend'] = 'HSI * 20000 - Small Mouth Bass';
   
   $thisgraph['bargraphs'][6]['graphrecs'] = $smb->logtable;
   $thisgraph['bargraphs'][6]['xcol'] = 'thisdate';
   $thisgraph['bargraphs'][6]['ycol'] = 'Vwq';
   $thisgraph['bargraphs'][6]['color'] = 'yellow';
   $thisgraph['bargraphs'][6]['yaxis'] = 2;
   $thisgraph['bargraphs'][6]['ylegend'] = 'Vwq - Small Mouth Bass (WQ)';
  
   $thisgraph['bargraphs'][4]['graphrecs'] = $ghsi->logtable;
   $thisgraph['bargraphs'][4]['xcol'] = 'thisdate';
   $thisgraph['bargraphs'][4]['ycol'] = 'pc';
   $thisgraph['bargraphs'][4]['color'] = 'brown';
   $thisgraph['bargraphs'][4]['yaxis'] = 2;
   $thisgraph['bargraphs'][4]['ylegend'] = 'WUA - Pool Cover';
  
   $thisgraph['bargraphs'][5]['graphrecs'] = $fishkills;
   $thisgraph['bargraphs'][5]['xcol'] = 'thisdate';
   $thisgraph['bargraphs'][5]['ycol'] = 'killreport';
   $thisgraph['bargraphs'][5]['color'] = 'purple';
   $thisgraph['bargraphs'][5]['yaxis'] = 2;
   $thisgraph['bargraphs'][5]['fillcolor'] = 'purple';
   $thisgraph['bargraphs'][5]['alpha'] = 0.5;
   $thisgraph['bargraphs'][5]['ylegend'] = 'Fish Kill Reported (1 or 0)';
...
/*   
   ...
   $thisgraph['bargraphs'][5]['graphrecs'] = $ghsi->logtable;
   $thisgraph['bargraphs'][5]['xcol'] = 'thisdate';
   ...
   $thisgraph['bargraphs'][6]['graphrecs'] = $rbt->logtable;
   $thisgraph['bargraphs'][6]['xcol'] = 'thisdate';
...
#   $debug = 1;
   ...
   $fdgraph['bargraphs'][0]['graphrecs'] = $gageduration;
   $fdgraph['bargraphs'][0]['xcol'] = 'time';
   $fdgraph['bargraphs'][0]['ycol'] = 'Qout';
   $fdgraph['bargraphs'][0]['color'] = 'blue';
   $fdgraph['bargraphs'][0]['ylegend'] = 'Observed';

   $fdgraph['bargraphs'][1]['graphrecs'] = $vtduration;
   $fdgraph['bargraphs'][1]['xcol'] = 'time';
   $fdgraph['bargraphs'][1]['ycol'] = 'Qout';
   $fdgraph['bargraphs'][1]['color'] = 'red';
   $fdgraph['bargraphs'][1]['ylegend'] = 'VT HSPF';

   $fdgraph['bargraphs'][2]['graphrecs'] = $cbpduration;
   $fdgraph['bargraphs'][2]['xcol'] = 'time';
   $fdgraph['bargraphs'][2]['ycol'] = 'Qout';
   $fdgraph['bargraphs'][2]['color'] = 'orange';
   $fdgraph['bargraphs'][2]['ylegend'] = 'CBP HSPF';
