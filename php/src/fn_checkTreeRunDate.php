...
if (isset($argv[1])) {
   ...
   print("Usage: php fn_checkTreeRunDate.php elementid runid [startdate] [enddate] [cachedate]\n");
   die;
}
if (isset($argv[2])) {
   $runid = $argv[2];
...
$enddate = isset($argv[3]) ? $argv[4] : '2005-12-31';
$cache_date = isset($argv[3]) ? $argv[5] : date('Y-m-d');
error_reporting(E_ALL);
print("Clearing elementid $elementid, Run - $runid \n");
$status = checkTreeRunDate($listobject, $elementid, $runid, $startdate, $enddate, $cache_date);
print("Status: $status.\n");
