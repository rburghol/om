...
function getElementInfo($listobject, $elementid) {
   $listobject->querystring = "  select elementid, scenarioid, objectclass, custom1, custom2 ";
   $listobject->querystring .= " where elementid = $elementid ";
   $listobject->performQuery();
   if ($listobject->numrows > 0) {
      return $litobject->queryrecords[0];
   } else {
      return array();
   }
}

function checkRunData($listobject, $runid, $startdate, $enddate, $outlets) {
   $msg_ok = '';
   $msg_bad = '';
   $msg_null = '';
   $bad_elements = array();
   ...
   foreach ($outlets as $thisrec) {
      ...
      if ($detail['exists']) {
         ...
         $unit = ($Qlocal / $Alocal);
         if ( ( $unit <= 0.5 ) or ($unit >= 2.0) ) {
            $msg_bad .= "Element $elid Failed Local Unit Flow Test $unit = ($Qlocal + WD[$wd] - PS[$ps]) / $Alocal<br>\n";
            $bad = 1;
         }
...
function getCOVARunDetail($listobject, $elid, $runid, $host = '') {
   ...
   if (is_array($run_rec)) {
      ...
      $runoff_rec = getRunFile($listobject, $cid, $runid);
      $runoff_file = $runoff_rec['output_file'];
      $info = verifyRunVars($runoff_file,array('demand_broadcast_mgd','Runit', 'local_area'),$quick_num);
      $output['demand_cfs'] = $info['demand_broadcast_mgd']['mean'] * 1.54;
...
function summarizeRun($listobject, $recid, $run_id, $startdate, $enddate, $force=0) {
   $output = array();
   $run_mesg = '';
   ...
   if ( (!checkSummary($listobject, $recid, $run_id)) or $force) {
      ...
      //print("Checking\n");
      $output = checkRunData($listobject, $run_id, $startdate, $enddate, $outlet);
      //print("Done Checking\n");
...
function shakeTree($listobject, $sip, $num_sim, $recid, $run_id, $startdate, $enddate, $cache_date, $debug = 0) {
   ...
   */
   ...
   summarizeRun($listobject, $recid, $run_id, $startdate, $enddate);
   ...
   foreach ($children as $thischild) {
      ...
      $check = shakeTree($listobject, $sip, $num_sim, $childid, $run_id, $startdate, $enddate, $cache_date, 0);
