...
global $templateid, $t_reachid, $pswd_group_tid, $lstemplateid, $sw_tid, $gw_tid, $ps_contid, $ps_tid, $projectid, $cbp_scen;
...
function getWithdrawalGroup($scenarioid, $listobject, $subwatershed_elid, $groupcat, $groupname, $g_template, $debug = 0) {
...
function createCOVAWithdrawalObjects($segs, $scenid, $projectid) {
   ...
   foreach ($segs as $this_segment) {
      ...
      $wd = getVWUDSWithdrawals($vwudsdb, $listobject, $parentid, 1);
      print_r($wd);
      print("<br>\n");
      ...
      foreach ($wd as $thiswd) {
         ...
         $gid = getWithdrawalGroup($scenid, $listobject, $parentid, $w_cat, $gname, $pswd_group_tid, 1);
         if ($gid) {
            $tid = -1;
            ...
            if ($tid > 0) {
               ...
               $recid = $output['elementid'];
               print("Setting properties on $recid \n" . print_r($props,1) . "\n");
               $props = array('name'=>$thiswd['facility'] . ' ' . $thiswd['SOURCE'] . '(' . $thiswd['mpid'] . ')', 'id1'=>$uid, 'id2'=>$mpid, 'custom2' =>$mpid, 'discharge_enabled'=>0, 'wdtype'=>$wtype);
               updateObjectProps($projectid, $recid, $props);
               $obres = unSerializeSingleModelObject($recid);
               $thisobject = $obres['object'];
               if (method_exists($thisobject, 'reCreate')) {
...
function createCOVAPSWDObjects($segs, $scenid, $projectid) {
   ...
   foreach ($segs as $this_segment) {
      ...
      $ps = getVPDESDischarges($vpdesdb, $listobject, $parentid, 1);
      print("\n VPDES records found \n");
...
function getCBPInfoByRiverseg($listobject, $riverseg, $debug = 0) {
   $retvals = array();
   $listobject->querystring = "select riverseg, watershed, rivername, max(contrib_area_sqmi) as contrib_area_sqmi, sum(area2d(transform(the_geom,26918)) * 3.86102159E-7) as local_area_sqmi from  sc_cbp53 where riverseg = '$riverseg' group by riverseg, watershed, rivername ";
   if ($debug) {
...
function getCOVADischarges($listobject, $parentid) {
...
function getCOVACBPLandsegPointContainer($latdd, $londd) {
...
function findExistingCOVAContainers($listobject, $scenarioid, $latdd, $londd) {
   ...
   $els = array();
   if (count($fromlist) > 0) {
       $els = getComponentCustom($listobject, $scenarioid, 'cova_ws_container', '', -1, $fromlist);
      // should screen for smallest one here, but for now, we just take the first one
      print("Found existing Subset of elements type 'cova_ws_container' that contain this point: \n");
      print_r($els);
      print("\n");
   }
   return $els;
...
function getChildCustomType($listobject, $parentid, $custom1, $custom2 = '') {
   // retrieves the va_hydro sub-component (va's HSPF cbp model)
   // when given the 'cova_runoff' containers ID
...
function getCOVADownstream($listobject, $parentid) {
...
function setCOVACBPReachProps($projectid, $cbp_listobject, $listobject, $riverseg, $cid, $debug) {

   $cbp_listobject->querystring = "   select  reach_len_mi * 5284.0 as reachlen_ft,  ";
   $cbp_listobject->querystring .= " ( reach_drop_ft / (reach_len_mi * 5284.0) ) as c_slope ";
   ...
   $listobject->querystring .= " max(contrib_area_sqmi) as contrib_area_sqmi from sc_cbp53 where riverseg = '$riverseg' group by riverseg ";
   print(" $listobject->querystring <br>\n");
   $listobject->performQuery();
   if ($listobject->numrows > 0) {
   ...
   $params = array('area'=>$areasqmi, 'drainage_area' => $contrib_area_sqmi, 'slope' => $c_slope, 'length' => $reachlen_ft);
...
function setLUMatrix ($elementid, $lumatrix_name, $lumatrix) {
   global $listobject;
   print("Trying to apply land use matrix: \n");
