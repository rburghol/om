...
if ($props['adminname'] == '') {
   if ($debug) {
      print("Looking for admin table name <br>");
   }
   foreach ($adminsetuparray as $thistype => $asrec) {
      if ($debug) {
         print(" ... Checking $thistype against " . $object_info['custom1']);
      }
      if (isset($asrec['table info']['object_custom1'])) {
         ...
         if (in_array($object_info['custom1'], $types)) {
            ...
            if ($debug) {
               print(" ... Setting admin type to $thistype<br>");
            }
            break;
         }
      }
...
if (isset($adminsetup['table info']['templatefile'])) {
   ...
   if ($debug) {
      print("Getting template setup from adminsetup record.<br>");
   }
}
if (isset($adminsetup['table info']['templateid'])) {
   $props['templateid'] = $adminsetup['table info']['templateid'];
   if ($debug) {
      print("Getting template id setup from adminsetup record.<br>");
   }
}
if (isset($adminsetup['table info']['runform'])) {
   $props['runform'] = $adminsetup['table info']['runform'];
   if ($debug) {
      print("Getting Run Form ( " . $props['runform'] . ") from adminsetup record.<br>");
   }
}
...
if ($debug) {
   print("ASET: " . $props['adminname'] . " - FILE: $filename - ASETrecs" . print_r($adminsetup,1) . "<br>");
}
...
if ( ($showel <> -1) and ($props['actiontype'] <> 'load_cached') ) {
   ...
   if ($debug) {
      print("getModelVarsForCustomForm($showel, " . $props['templatefile'] . ", $debug);<br>");
...
if ($props['actiontype'] == 'list') {
   ...
   if ($debug) {
      print("Setting up tab $tabname from file " . $props['templatefile'] . " <br>");
   }
   $content = file_get_contents($props['templatefile']);
   if (!$formerror) {
...
switch ($props['runform']) {
   case 'vwp':
      ...
      $taboutput->tab_HTML['model_run'] .= showGenericButton('refresh', 'Update Model Status', "xajax_showStatus(xajax.getFormValues(\"runinfo\"));", 1) . "<br>";
...
// *****************************
$searchvars = $_POST;
$taboutput->tab_HTML['browse'] .= "<h3>Browse Projects</h3><br>";
if (isset($adminsetup['table info']['object_custom1'])) {
   ...
   $othsql .= "    and ( ( a.groupid in ($usergroupids) and a.gperms >= 4 ) ";
   $othsql .= "       or (a.pperms >= 4) ";
   $othsql .= "    ) ";
   $othsql .= "    and a.ownerid <> $userid ";
   $othsql .= " order by a.elemname ";
   
   foreach (array("My Projects"=>$obsql, "Other Projects"=>$othsql) as $key => $sql) {
