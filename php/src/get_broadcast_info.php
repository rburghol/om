...
if ( (count($c_casts) == 0) and (count($p_casts) == 0)) {
   ...
   if ($pid > 0) {
      $pname = getElementName($listobject, $pid);
   ...
   $missing_html = '';
   $missing_html .= "<b>Children NOT in this broadcast</b><table><tr><td>";
   
   $cclasses = array_keys($c_casts);
   //$chtml .= print_r($cclasses,1);
   $childrecs = getChildComponentType($listobject, $elid);
   $chtml .= "<b>All Children:</b><br><table>";
   foreach ($childrecs as $thisrec) {
      $cid = $thisrec['elementid'];
      $cname = $thisrec['elemname'];
      if (!in_array($cid, $children_present)) {
         //$child_casts = getBroadCasts($cid, 'parent', '', '');
         $missing_html .= $mdel . " <a href='$scriptname?elementid=$cid'>$cname ($cid)</a>";
         $mdel = ",";
      }
      $chtml .= "<tr><td> Child $cname ($cid) <bt>";
      $chtml .= "<a href='$scriptname?elementid=$cid'>Show Broadcasts</a>";
      $chtml .= "</td></tr>";
   }
   $chtml .= "</table>";
   $chtml .= "</tr></table>";
