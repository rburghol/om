...
function model_menuEditItem($listobject, $formname, $elementid, $branchid, $name, $scid, $icon, $view) {
   ...
   /*
   // no need for this since clone does this by default??
   if (count($ob['children']) > 0) {
      # add a deep cloning icon
      // this one futsed up the active scenario
      $deepclone = array(
         'name'=>'Deep Clone',
         'label'=>'Deep Clone',
         'icon'=>$icons['deepclone'],
         'onClick'=>"last_tab['model_element']='model_element_data0'; last_button['model_element']='model_element_0'; last_tab['modelout']='modelout_data0'; last_button['modelout']='modelout_0';  show_next('map_window_data0', 'map_window_0', 'map_window'); document.forms['$formname'].elements.elementid.value=$branchid;  document.forms['$formname'].elements.actiontype.value='deepclone'; xajax_insertGroupClone(xajax.getFormValues('$formname')); ", 
...
function showSystemToolsMenu($listobject, $projectid, $scenarioid, $userid, $usergroupids, $debug) {
   ...
   $menu_names = array(
      ...
      "Modeling Activity Viewer"=>"last_tab['model_element']='model_element_data0'; last_button['model_element']='model_element_0'; last_tab['modelout']='modelout_data0'; last_button['modelout']='modelout_0'; show_next('map_window_data0', 'map_window_0', 'map_window'); xajax_showModelActivity(xajax.getFormValues('elementtree'));",
...
function getCBPTotalContribArea($listobject, $tablename, $colname, $areacolname, $gcolname, $srid, $cfact, $segid, $debug) {
   ...
   $area = 0;
   $listobject->querystring = " select $cfact * st_area2d(st_transform($gcolname,$srid)) as localarea from $tablename where split_part($colname, '_', 2) = '$segid' ";
   if ($debug) {
      print("$listobject->querystring ; <br>");
   }
   $listobject->performQuery();
...
function getYearsFromLU($lu_recs) {
   $yr_keys = array_keys($lu_recs[0]);
   $yrs = array();
   foreach ($yr_keys as $thiskey) {
      if (!($thiskey == 'luarea')) {
         $yrs[] = $thiskey;
      }
   }
...
function extractYearLU($lu_recs, $year = 'max', $dest_year = -1) {
   // extract a specific year from the LU data set
   $oneyearlu = array();
   $yrs = getYearsFromLU($lu_recs);
   print("Found years: " . print_r($yrs,1) . "\n");
   switch ($year) {
      case 'min':
         $thisyear = min($yrs);
      break;
   ...
   print("Searching for land use year $thisyear \n");
   
   foreach ($lu_recs as $thisrec) {
      $luname = $thisrec['luname'];
      $luarea = $thisrec[$thisyear];
      $oneyearlu[] = array('luname'=>$luname, $dest_year => $luarea);
   }
   
   return $oneyearlu;
}

function adjustLandUses($lu_recs, $dest_lus, $target_pct, $operation = 'min', $src_lus = array(), $decimal_places = -1) {
   // will adjust the dest_lus proportionally so that they are in line with the target_pct 
   // if src_lus is empty, it will operate on all lu's except target
   // get dest_lus
   $ret_lus = array();
   if (count($dest_lus) == 0) {
      return $lu_recs;
   ...
   $yrs = getYearsFromLU($lu_recs);
   $thisyear = max($yrs);
   print("Year to convert is $thisyear \n");
   $src_total = 0;
   $dest_total = 0;
   foreach ($lu_recs as $thisrec) {
      $luname = $thisrec['luname'];
      $luarea = $thisrec[$thisyear];
      if ($luname <> 'wat') {
         if (in_array($luname, $dest_lus)) {
            $dest_total += $luarea;
         } else {
            if ( in_array($luname, $src_lus ) or (count($src_lus) == 0) ) {
               $src_total += $luarea;
            }
         }
         $all += $luarea;
   ...
   $dest_pct = $dest_total / $all; // current percent destination landuse
   // now, only process if the dest LUs are more/less than the desired operation
   $do_nothing = 0;
   switch ($operation) {
      case 'min':
      if ($dest_pct >= $target_pct) {
         $do_nothing = 1;
         print(" $dest_pct >= $target_pct - no transformation required \n");
      }
      break;
   ...
   if (!$do_nothing) {
      $dest_amt = $target_pct * $all;
      $dest_mult = $dest_amt / $dest_total;
      $src_amt = $src_total + ($dest_total - $dest_amt);
      $src_mult = $src_amt / $src_total;
      print("Source Total = $src_total \n");
      print("Source multiplier = $src_mult \n");
      print("Destination Existing = $dest_total \n");
      print("Destination multiplier = $dest_mult \n");
      print("Rounding to $decimal_places decimal places \n");
      foreach ($lu_recs as $thisrec) {
         $luname = $thisrec['luname'];
         $luarea = $thisrec[$thisyear];
         if ($luname <> 'wat') {
            if (in_array($luname, $dest_lus)) {
               print("Multiplying destination lu $luname = $dest_mult * $luarea \n");
               $luarea = $dest_mult * $luarea;
            } else {
               if ( in_array($luname, $src_lus ) or (count($src_lus) == 0) ) {
                  print("Multiplying source lu $luname = $src_mult * $luarea \n");
                  $luarea = $src_mult * $luarea;
               }
            }
         }
         if ($decimal_places > 0) {
            $lurnd = round($luarea,$decimal_places);
         } else {
            $lurnd = $luarea;
         }
         $ret_lus[] = array('luname'=>$luname, $thisyear => $lurnd);
      }
   } else {
      $ret_lus = $lu_recs;
   }
...
function getCBPLandSegIntersection($dbobj, $wktgeom, $fipsab = -1) {
   $dbobj->querystring = "  select fipsab,  ";
   $dbobj->querystring .= "    st_asText(st_memgeomunion(st_intersection(st_setsrid(st_geomfromtext('$wktgeom'),4326),the_geom))) as wktgeom ";
   $dbobj->querystring .= " from gis_cbp_landsegs_dd ";
   $dbobj->querystring .= " where st_setsrid(st_geomfromtext('$wktgeom'),4326) && the_geom ";
   $dbobj->querystring .= "    and st_area2d(st_intersection(st_setsrid(st_geomfromtext('$wktgeom'),4326),the_geom)) > 0 ";
   $dbobj->querystring .= "    and ( (fipsab = '$fipsab') or ($fipsab = -1) ) ";
   $dbobj->querystring .= " group by fipsab ";
   $dbobj->performQuery();
   //print($dbobj->querystring);
   
   if ($dbobj->numrows > 0) {
      return $dbobj->queryrecords;
   } else {
      return FALSE;
   }
...
function getCBPSegList($listobject, $tablename, $colname, $segid, $debug, $max_levels=-1, $criteria = array()) {
   ...
   $trib_segs['headwaters'] = array();
   $trib_segs['headwater_names'] = array();
   $trib_segs['info'] = '';
   ...
   if ( ($max_levels > 0) or ($max_levels == -1) ) {
      ...
      foreach ($qrecs as $thisrec) {
         ...
         if ($tribsegid <> $segid) {
            ...
            $tmp_hws = array_merge($trib_segs['headwaters'], $upstream_segs['headwaters']);
            $tmp_hwns = array_merge($trib_segs['headwater_names'], $upstream_segs['headwater_names']);
            $trib_segs['segments'] = $tmp_segs;
            $trib_segs['segnames'] = $tmp_segnames;
            $trib_segs['headwaters'] = $tmp_hws;
            $trib_segs['headwater_names'] = $tmp_hwns;
            $trib_segs['info'] .= $upstream_segs['info'];
         } else {
            $trib_segs['info'] .= "$segid is a headwater segment.<br>";
            error_log("$segid is a headwater segment.<br>");
            $trib_segs['headwaters'][] = $segid;
            $trib_segs['headwater_names'][] = $segname;
         }
      }
   }
   if (count($trib_segs['segments']) == 1) {
      if ($trib_segs['segments'][0] == $segid) {
         $trib_segs['headwaters'][] = $segid;
         $trib_segs['headwater_names'][] = $segname;
      }
   }
...
function getDownstreamSegment($listobject, $segment, $tablename = 'sc_cbp53', $riverseg_col = 'riverseg', $criteria = array(), $debug = 0) {
   # user passes uin tablename and colname just so this can be flexible if we should ever have a 
   # new table with the CBP data in it
   $ds_nodeinfo = array('debug'=>'', 'riversegment'=>'', 'usegid'=>'', 'dsegid'=>'');
   $listobject->querystring = "  select a.\"$riverseg_col\" as riversegment, substring(a.\"$riverseg_col\",5,4) as dsegid, ";
   $listobject->querystring .= "  b.\"$riverseg_col\" as usegment, substring(b.\"$riverseg_col\",5,4) as usegid ";
   $listobject->querystring .= " from \"$tablename\" as a, \"$tablename\" as b ";
   $listobject->querystring .= " where substring(a.\"$riverseg_col\",5,4) = substring(b.\"$riverseg_col\",10,4) ";
   if (strlen($segment) == 4) {
      $listobject->querystring .= "    AND substring(b.\"$riverseg_col\",5,4) = '$segment' ";
   } else {
      $listobject->querystring .= "    AND b.\"$riverseg_col\" = '$segment' ";
   }
   
   $ds_nodeinfo['debug'] .= "$listobject->querystring ; <br>";
   
   $listobject->performQuery();
   $qrecs = $listobject->queryrecords;
   if ($listobject->numrows > 0) {
      $ds_nodeinfo['riversegment'] = $listobject->getRecordValue(1,'riversegment');
      $ds_nodeinfo['usegment'] = $listobject->getRecordValue(1,'usegment');
      $ds_nodeinfo['dsegid'] = $listobject->getRecordValue(1,'dsegid');
      $ds_nodeinfo['usegid'] = $listobject->getRecordValue(1,'usegid');
   }
   return $ds_nodeinfo;   
   
}

function getStreamSection($listobject, $start, $end, $tablename = 'sc_cbp53', $riverseg_col = 'riverseg', $criteria = array(), $debug = 0) {
   $segments = array($start);
   $thisone = $start;
   $lastone = -1;
   // gets from start point to end point in river chain
   while ( ($thisone <> $end) and ($thisone <> $lastone) and ($thisone <> '') ) {
      $info = getDownstreamSegment($listobject, $thisone);
      $lastone = $thisone;
      $thisone = $info['riversegment'];
      if (count($segments) == 1) {
         // first segment, capture the full segment name
         $segments[0] = $info['usegment'];
      }
      $segments[] = $thisone;
   }
   return $segments;
