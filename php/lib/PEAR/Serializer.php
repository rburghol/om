...
class XML_Serializer extends PEAR
{
    ...
    */
    function _serializeObject(&$object, $tagName = null, $attributes = array())
    {
        ...
        # original code treats all properties as array
        $string = $this->_serializeArray($properties, $tagName, $attributes);
        /* 
        # new code, checks each property, handles it independently
        $string = '';
        foreach ($properties as $thisprop) {
           if (is_array($thisprop)) {
               ...
               $string .= $this->_serializeArray($thisprop, $tagName, $rootAttributes);
           } elseif (is_object($thisprop)) {
               // serialize an object
               if (isset($this->options[XML_SERIALIZER_OPTION_ROOT_NAME])) {
                   $tagName = $this->options[XML_SERIALIZER_OPTION_ROOT_NAME];
               } else {
                   $tagName = get_class($thisprop);
               }
               $string .= $this->_serializeObject($thisprop, $tagName, $rootAttributes);
           } else {
               $tag = array();
               if (isset($this->options[XML_SERIALIZER_OPTION_ROOT_NAME])) {
                   $tag['qname'] = $this->options[XML_SERIALIZER_OPTION_ROOT_NAME];
               } else {
                   $tag['qname'] = gettype($thisprop);
               }
               if ($this->options[XML_SERIALIZER_OPTION_TYPEHINTS] === true) {
                   $rootAttributes[$this->options[XML_SERIALIZER_OPTION_ATTRIBUTE_TYPE]] = gettype($thisprop);
               }
               @settype($thisprop, 'string');
               $tag['content']    = $thisprop;
               $tag['attributes'] = $rootAttributes;
               $string .= $this->_createXMLTag($tag);
           }
        }
        */
        return $string;
