...
Qdaily = aggregate(as.numeric(dat$Qout), as.list(dat$thisdate), FUN = mean);
Qdaily = zoo(data.frame('Qdaily' = Qdaily$x), order.by = Qdaily$Group.1);
...
ymax = ceiling(max(as.numeric(wudat$Qzero)));
#wudat <- window(dat, start = as.Date("1989-05-06"), end = as.Date("1989-05-08 23:59:59"));
#wudaily <-  window(Qdaily, start = as.Date("1989-05-06"), end = as.Date("1989-05-08"));
wudat <- window(dat, start = as.POSIXct("1989-05-06"), end = as.POSIXct("1989-05-08 23:59:59"));
wudaily <-  window(Qdaily, start = as.POSIXct("1989-05-06"), end = as.POSIXct("1989-05-08"));
wudat$Qdaily = 0
# wudat[index(match(wudat$thisdate, index(wudaily)))]$Qdaily = wudaily[match(wudat$thisdate, index(wudaily))]
wudat[,]$Qdaily = as.vector(wudaily[match(wudat$thisdate, index(wudaily))])
...
par(pch=22, col="orange") # plotting symbol and color 
lines(wudat$impqoundment_riser_Qout)

plot(dat$impoundment_lake_elev,ylim=c(0,20))
