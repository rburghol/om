...
runids = c(20021,20024);
eventstart = "2002-02-15"
eventend = "2002-06-15"
targetyear = 2002;
bigQ = 50;
...
for (rix in 1:length(runids)) {
  ...
  dat <- fn_get_runfile(elid, runid,  cached = FALSE);
  ...
  dat.pre <- zoo(as.numeric(dat$impoundment_Qin), order.by = dat$timestamp)
  dat.post <- zoo(as.numeric(dat$impoundment_Qout), order.by = dat$timestamp)
  pulse.pre = group4(dat.pre)
  pulse.post = group4(dat.post)
  preym = max(pulse.pre[,"High pulse length"], na.rm= TRUE)
  postym = max(pulse.post[,"High pulse length"], na.rm= TRUE)
  ym = max(c(preym,postym))
  boxplot(
    pulse.pre[,"High pulse length"], 
    pulse.post[,"High pulse length"], 
    labels = c("Pre CMP", "CMP"),
    names = c("Pre CMP", "CMP"),
    main = paste("High Pulse Length, RunID ", runid),
