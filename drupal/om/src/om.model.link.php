...
$link_type = 1; // basic child to parent linkage 
$prop_varkey = 'om_map_model_linkage'; // the type of 
...
if ( (count($args) >= 4) or ($args[0] == 'file')) {
  ...
  $src_id = $args[2];
  $link_type = $args[3];
  $src_prop = isset($args[4]) ? $args(4) : FALSE;
  $src_prop = isset($args[4]) ? $args(4) : FALSE;
  $src_prop = isset($args[4]) ? $args(4) : FALSE;
  $src_prop = isset($args[4]) ? $args(4) : FALSE;
  $dest_prop = $args[5];
  $src_entity_type = $args[6];
  $dest_entity_type = $args[7];
} else {
  // warn and quit
