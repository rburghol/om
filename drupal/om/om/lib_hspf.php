...
class HSPF_UCIobject {
   ...
   function initAllFromUCI() {
      ...
      if ($this->debug) { 
         print("Parsing UCI.<br>");
      }
      #print_r($this->ucitables);
      ...
      foreach ($geninfoblocks as $thisblock) {
         ...
         if (in_array($blockname, array_keys($this->ucitables))) {
            ...
            $this->listobject->querystring = " update uciblocks set ucitabletag = '$blockname' where section = '$block' and subsection = '$subblock' ";
            $tt = $this->listobject->querystring;
            #print("$tt<br>");
            $this->listobject->performQuery();
         } else {
            if ($this->debug) { 
               print("$block - $subblock - $blockname not found in ucitables array, section ignored. <br>");
            }
         }
      }
      ...
      foreach ($otherblocks as $thisblock) {
         ...
         if ($this->debug) { print("Parsing Block: $block - $subblock<br>"); }

         if (!(in_array($subblockname, array_keys($this->ucitables)))) {
            # try other name combinations
            switch ($block) {
               case 'PERLND':
         ...
         if (in_array($subblockname, array_keys($this->ucitables))) {
            ...
            $this->listobject->querystring = " update uciblocks set ucitabletag = '$subblockname' where section = '$block' and subsection = '$subblock' ";
            $tt = $this->listobject->querystring;
            #print("$tt<br>");
            $this->listobject->performQuery();
         } else {
            if ($this->debug) { 
               print("$block - $subblock not found in ucitables array, section ignored. <br>");
            }
         }
      }
...
function outputGroupedUCIpart($thisucipart) {
  ...
  function getUCIBlocks() {
     ...
     if ($this->listobject) {
        $this->listobject->querystring = 'create temp table uciblocks(section varchar(64), subsection varchar(64), subsectionid varchar(64), ucitabletag varchar(128))';
        $this->listobject->performQuery();
     }
...
function otherfunction($outfile,$dbcols,$fielddelim) {
   ...
   $varlist = explode(",",$dbcols);
   $valuelist = explode($fielddelim,$inline);
...
function makeUCITable($dbobj,$ucisection,$datalines,$ucitables,$debug) {
   ...
   $paramdefaults = $tableinfo['paramdefaults'];
   $paramar = explode(',', $paramdefaults);
   ...
   foreach($datalines as $thisline) {
      $cleanar = array();
      $j = 0;
      foreach ($thisline as $thisentry) {
         $cleanval = rtrim(ltrim($thisentry));
         # we should try to load default params here, but so far, this is tricky due to the 
         # fact that we formed the config array such that there were no slots for default params 
         # for the segment start and ending columns -- probably a bad choice.
         # therefore, comment these out:
         /*
         if ( ($cleanval == '') and ( count($paramar) >= ($j + 1) ) ) {
            $cleanval = $paramar[$j];
         }
         */
         # and instead, we put in a hack to assume that if the UCI leaves the segend column blank
         # we make segend equivalent to segstart
         if ($j == 1) {
            if ($cleanval == '') {
               if (strstr($tabledef, 'segend')) {
                  # this table has the segend param
                  $cleanval = $cleanar[0];
               }
            }
         }
      ...
      array_walk($cleanar,'listToSQL');
      #print_r($cleanar);
      $testreplace = array('testval'=>"''", 'replaceval'=>'NULL');
      array_walk($cleanar, 'arrayReplace', $testreplace);
      $linevals = implode(",",$cleanar);
...
function createWDMUtilExpFormat($dataarray,$datefield,$datafield,$location,$datatype,$scenario,$description) {
   ...
   foreach ($dataarray as $dataline) {
      ...
      /*
         ...
         $year = $thisdate['year'];
      */
      list($year,$mo,$daytime) = explode('-',$datecol);
...
function createWDIMEXDSN($dsn,$tstype,$tc,$dsndata,$datecol,$datacol,$mindate,$maxdate) {
  ...
  list($maxyear,$mo,$daytime) = explode('-',$maxdate);
  list($minyear,$mo,$daytime) = explode('-',$mindate);
  ...
  foreach($dsndata as $thisline) {
     ...
     list($year,$mo,$daytime) = explode('-',$thisdate);
     list($day,$time) = explode(' ',$daytime);
...
if (strlen($wdmfile) > 0) {
   ...
   while (count($mutfiles) > 0) {
      ...
      for ($i = 30;( ($i < 99) and (count($mutfiles) > 0) ) ;$i++) {
         ...
         list($filebase,$ext) = explode("\.",$thismut);
