...
##########################################################
##                pgsql_QueryObject-dwrapper            ##
## wrapper around Drupal db  - temp solution
##########################################################


class pgsql_QueryObject_dwrapper {
  ...
  function performQuery() {
    ...
    if ( ($this->querystring <> "") && (isset($this->dbconn)) ) {
      ...
      $this->error = FALSE;
      $this->result = db_query($this->querystring);
      if (method_exists($this->result,'errorCode') {
       $this->error = $this->result->errorCode();
      } else {
       drupal_set_message("errorCode method does not exist on result");
       $this->error = false;
      }
      ...
      if (!$this->error) {
        ...
        while ($array = $result->fetchAssoc()) {
          $this->queryrecords[] = $array;
          $this->numrows++;
          if ($this->numrows++ >= $this->maxrecords) {
            drupal_set_message("Max records exceeded in query, result stored.");
            break;
          }
        }
        $this->lastserial = -1;
        ...
        if (isset($this->adminsetup['table info']['pk_seq'])) {
           ...
           $this->lastserial = db_query("SELECT currval('$seqname')")->fetchField();
        }
      }
